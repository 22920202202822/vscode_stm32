...
/**
  ...
  */
/*
  uart1 for vision serial
  uart2 for remote serial
  uart4 for imu serial
  uart6 for judgement serial
*/
...
/* USER CODE BEGIN 0 */
#include "stdarg.h"
uint8_t UartTxBuf[128]; // use for Uartprintf
uint8_t uart1_rx_buf[uart1_buf_len];
uint8_t uart2_rx_buf[uart2_buf_len];
uint8_t uart4_rx_buf[uart4_buf_len];
uint8_t uart6_rx_buf[uart6_buf_len];
/* USER CODE END 0 */
...
void MX_UART4_Init(void)
{
  ...
  HAL_UART_Receive_DMA(&huart4, uart4_rx_buf, uart4_buf_len);
...
void MX_USART1_UART_Init(void)
{
  ...
  HAL_UART_Receive_DMA(&huart1, uart1_rx_buf, uart1_buf_len);
...
void MX_USART2_UART_Init(void)
{
  ...
  HAL_UART_Receive_DMA(&huart2, uart2_rx_buf, uart2_buf_len);
...
void MX_USART6_UART_Init(void)
{
  ...
  HAL_UART_Receive_DMA(&huart6, uart6_rx_buf, uart6_buf_len);
...
{
  if(SelectUART == UART2_print)
  {
    HAL_UART_Transmit_DMA(&huart2, buf, len);
    return true;
  }
  else if(SelectUART == UART4_print)
  {
    HAL_UART_Transmit_DMA(&huart4, buf, len);
  }
  else if(SelectUART == UART1_print)
  {
    HAL_UART_Transmit_DMA(&huart1, buf, len);
  }
  else if(SelectUART == UART6_print)
  {
...
bool Transmit(uint8_t SelectUART, uint8_t* buf, int len)
{
  if(SelectUART == UART2_print)
  {
    HAL_UART_Transmit(&huart2, buf, len, 1); // 1 is timeout
    return true;
  }
  else if(SelectUART == UART4_print)
  {
    HAL_UART_Transmit(&huart4, buf, len, 1);
  }
  else if(SelectUART == UART1_print)
  {
    HAL_UART_Transmit(&huart1, buf, len, 1);
  }
  else if(SelectUART == UART6_print)
  {
...
void UartPrintf(uint8_t SelectUART, const char *format,...)
{
	...
	va_end(args);
  if(SelectUART == UART2_print)
	  HAL_UART_Transmit(&huart2, UartTxBuf, len, 1); // 1 is timeout
  else if(SelectUART == UART1_print)
	  HAL_UART_Transmit(&huart1, UartTxBuf, len, 1); // 1 is timeout
  else if(SelectUART == UART4_print)
	  HAL_UART_Transmit(&huart4, UartTxBuf, len, 1);
  else if(SelectUART == UART6_print)
	  HAL_UART_Transmit(&huart6, UartTxBuf, len, 1);
}
// RoboMaster Judgement CRC8 check
...
// RoboMaster Judgement CRC16 check
